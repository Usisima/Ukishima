...
function _libExtractVibrant(imgEl) {
  try {
    ...
    var STEP = 30, S_PAL = 40, L_PAL = 48, BINS = 360/STEP;
    ...
    for (var i=0;i<px.length;i+=4) {
      ...
      if (h<0) h+=360;
      binW[Math.round(h/STEP)%BINS] += s;
    }
    var best=0, bw=binW[0];
    for (var b=1;b<BINS;b++) if (binW[b]>bw){bw=binW[b];best=b;}
    if (bw<=0) return null;
    return _libHslToHex(best*STEP, S_PAL, L_PAL);
  } catch(e){ return null; }
}
...
function _colorizeLibCovers() {
  var vc;
  try { vc = JSON.parse(localStorage.getItem('ukishima_vibrant3')||'{}'); } catch(e){ vc={}; }
  var pending = 0;

  function save() {
    try { localStorage.setItem('ukishima_vibrant3', JSON.stringify(vc)); } catch(e){}
  }
...
function palColor(subj) {
  ...
  if (icon) {
    try {
      var hex = JSON.parse(localStorage.getItem('ukishima_vibrant3') || '{}')[icon];
      if (hex) {
  ...
  function flushEnv() {
    ...
    if (map) {
      ...
      const item = {
        type:   map.t,
        tipo:   map.label,
        label:  tit || (num ? `${map.label} ${num}` : map.label),
